...
grammar = pickle.load(open('./grammar_smooth_unk.pkl', 'rb'))
wordList = pickle.load(open('./wordList_smooth_unk.pkl', 'rb'))
N = pickle.load(open('./N_smooth_unk.pkl', 'rb'))

inpFile = open('./dev.strings', 'r')
outFile = open('./dev.parses_smooth_unk', 'w')
            ...
            for k in range(i+1, j):
                ...
                for Y in N:
                    for Z in N:
                        ...
                        if (Y is not 'TOP') or (Z is not 'TOP'):
                            for X, dictionary in grammar.items():
                                if math.log10(grammar[X][YZ])+best[i][k][Y]+best[k][j][Z]>best[i][j][X]:
                                    best[i][j][X] = math.log10(grammar[X][YZ])+best[i][k][Y]+best[k][j][Z]
